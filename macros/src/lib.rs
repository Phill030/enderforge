...
pub fn derive_streamable(input: TokenStream) -> TokenStream {
    ...
    match ast.data {
        Data::Struct(data_struct) => match data_struct.fields {
            Fields::Named(fields) => {
                let field_names_cloned = fields.named.iter().map(|field| &field.ident);
                ...
                let gen = quote! {
                    ...
                    impl crate::packets::packet::Packet for #struct_name {
                        async fn to_bytes(&self) -> Result<Vec<u8>, crate::errors::EncodeError> {
                            let mut buffer = vec![];

                            #(self.#field_names_cloned.encode(&mut buffer).await?;)*
